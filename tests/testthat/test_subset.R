...
test_that("subset by circle", {
    ...
    N <- sum(D <= 100)
    indexc <- expect_message(
        subset(index, circle = list(longitude = -77.06, latitude = 26.54, radius = 100)),
        paste("Kept", N, "cycles")
    )
})
...
test_that("subset by rectangle", {
    ...
    indexr <- expect_message(
        subset(index, rectangle = list(longitude = c(-77, -76), latitude = c(25, 26))),
        paste("Kept", N, "cycles")
    )
})
...
test_that("subset by polygon", {
    ...
    N <- nrow(intersection)
    expect_message(
        subset(index, polygon = list(latitude = c(25, 27, 25), longitude = c(-78, -77, -74))),
        paste("Kept", N, "cycles")
    )
})
...
test_that("subset by time", {
    ...
    indext <- expect_message(
        subset(index, time = list(from = from, to = to)),
        paste("Kept", N, "cycles")
    )
})
...
test_that("subset by institution", {
    ...
    indexi <- expect_message(
        subset(index, institution = "AO"),
        paste("Kept", N, "cycles")
    )
})
...
test_that("subset by float ID", {
    N <- sum(ID == "1901584")
    indexID <- expect_message(
        subset(index, ID = "1901584"),
        paste("Kept", N, "cycles")
    )
})
...
test_that("subset by dataMode", {
    ...
    index1 <- expect_message(
        subset(index, dataMode = "delayed"),
        paste("Kept", Ndelayed, "cycles")
    )
    Nrealtime <- sum(grepl(".*R[0-9_abc]+.nc$", index[["file"]]))
    index2 <- expect_message(
        subset(index, dataMode = "realtime"),
...
test_that("subset by parameter", {
    ...
    index1 <- expect_message(
        subset(indexBgc, parameter = "DOXY"),
        paste("Kept", N, "cycles")
    )
})
