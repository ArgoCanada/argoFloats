...
I thought that after rebuilding the package and its vignttes, and installing
the package, using
```{r eval=FALSE}
library(argoFloats)
load_all()
...
A useful parameter to familiarize with is the `dataStateIndicator`. This tells the user the the degree of processing the data has passed through and can be helpful to determine the validity of the data. `argoFloats` has incorporated a way to subset by `dataStateIndicator` to allow the user to analyze data with a specific dgree of validity if they wish.
...
`useAdjusted()` is a somewhat sophisticated process, as is explained in the
documentation for `useAdjusted()` and illustrated the following flowchart.

![Figure 4: Flow chart for `useAdjusted()`.](../man/figures/useAdjustedDiagram.png)
...
1. Carval, Thierry, Bob Keeley, Yasushi Takatsuki, Takashi Yoshida, Stephen Loch Loch, Claudia Schmid, and Roger Goldsmith. Argo User's Manual V3.3. Ifremer, 2019. \doi{10.13155/29825}
