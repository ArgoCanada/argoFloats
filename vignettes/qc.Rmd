...
Using the same example as shown in `plot(..., which='QC')`, and variables defined in the previous code, an example of 
...
Using the same example as shown in `plot(...,which='QC')` and `showQCTests()`, and variables defined in the previous code, the below example demonstrates the significance of `applyQC` with a TS diagram.
...
* I changed the graph to illustrate better that the adjustment is a simple
linear transformation.

* This code block was using a built-in file, not the file stated in its
caption, so the caption was wrong.

* There is no need for a pre-built PNG with a built-in dataset (since no
downloading is required, meaning that CRAN can handle this), so we can save
some space in the package.

* This code block was marked as do-not-execute, which is risky because it means
that the user (and we) cannot rely on it still working in future.

* Notice how I demonstrated here how to make fancy labels, with units
(containing Greek letters)

* I used italics in the figure number, which seems to make it stand out better.
I also ended the caption with a period.  Let's discuss whether to change
throughout.
