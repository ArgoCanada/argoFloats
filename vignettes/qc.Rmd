...
flag value of 9 are missing, and those given flags 3, 5, 6, and 7
are considered 'bad'.
...
id <- '1901584'
```
An example of how to get the `QC` plot for temperature for the float with `id`
equal to `r id` is as follows:
...
simple, and some study of the Argo documention will be required of any user who
...
and therefore very likely to be erronous.  A good step at this point might be
to find the individual cycles/profiles containing those stray points.
...
`summary(model)`, tells us that the p-value is < 2.2e-16, suggesting that the
user rejects the null hypothesis of equality between raw and adjusted
values.
