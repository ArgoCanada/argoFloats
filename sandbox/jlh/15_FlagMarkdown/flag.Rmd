...
reliable source. At first glance, however, it can be difficult for users to
understand the Quality Control (QC) completed on profiles. The creators of
`argoFloats` develped a way to therefore deal with flags and adjusted data
starting with a three step process to deal with flags involving
`plot(which='QC')`, `showQCTests()`and `applyQC()`. In addition to flag control,
`argoFloats` also deals with adjusted data in `useAdjusted()`.
...
specifically, testing is done in three levels [1]: 1: Real time that complete
checks on all measurements and assigned a qualityflag within a 24-48 hour
timeframe, 2. Delayed mode, and 3. Regional scientific analyses of all float
...
This vignette will walk the users through the three step process using the float
...
An example of how to get the QC plot for temperature, or "TEMP" for the float ID
= 1901584 is as follows:
...
this contenxt, the hex digits come from `HISTORY_QCTEST` of a single argoFloats
...
[2], we see that test two is the "Imposible data test".

*Exercise Two*: Use the first float from ID 4900845 in exercise one,to determine
which tests were performed and/or failed during quality control testing. EXPLAIN
YOUR RESULTS.

### Step Three: applyQC()

The `applyQC()` function examines quality-control (QC) within an argoFloats
obect that was reated by `readProfiles(). By default, it replaced all suspecious
...
also incorporated the idea of `<PARAM>_ADJUSTED` vs `<PARAM>_UNADJUSTED` which
considers "bad" data that has been adjusted [2].

Variables with original names indicating in the string `<PARAM>_ADJUSTED` are
assigned nicknames names ending in Adjusted by `readProfiles()`, so that e.g.
`DOXY_ADJUSTED` gets the nickname `oxygenAdjusted`, while `DOXY` gets the
...
*Exercise Four* Use `useAdjusted()` on the float ID=5903586, profile=50 to
compare `<PARAM>_ADJUSTED` vs `<PARAM>_UNADUSTED>`.
...
"bad" data are the same. This indicates to the user that it is okay to use the
data, even though QC tests were not performed, as shown by `showQCTests()`.
