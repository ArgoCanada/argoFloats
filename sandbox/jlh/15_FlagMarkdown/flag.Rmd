...
As described by the Argo user’s manual [1], data that are given a flag value of 1, 2, 5, or 8 are considered 'good' data. If data are given a flag value of 9 (or missing value) they are not used, and flags 3, 5, 6, and 7 are considered 'bad' data.
...
This vignette will walk the users through the three step process using the float ID = 1901584 near the Bahamas.
...
In section 3.11 Reference table 11: QC test binary IDs of the Argo User's Manual [1], we see that test two is the "Imposible data test".

*Exercise Two*: Use the first float from ID 4900845 in exercise one,to determine which tests were performed and/or failed during quality control testing. EXPLAIN YOUR RESULTS.
...
*Exercise Three* : Use the following code to look at the `<PARAM>_ADJUSTED vs '<PARAM>`. EXPLAIN YOUR RESULTS.
...
plot(clean, which="TS")
```
As shown by the plot, the plot containing "good" data and the plot containing "bad" data are the same. This indicates to the user that it is okay to use the data, even though QC tests were not performed, as shown by `showQCTests()`.


 FIX ME:::: 
Exercise Four (maybe):
This could be a result of no QC tests being performed on the float ID, as shown by `showQCTests()`, or a result of the data being considered "good" even in the raw, or unadjusted form.
