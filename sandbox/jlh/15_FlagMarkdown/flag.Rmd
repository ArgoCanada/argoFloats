...
specifically, testing is done in three levels [2]:

1: Real time that complete checks on all measurements and assigned a quality
flag within a 24-48 hour timeframe
2. Delayed mode
3. Regional scientific analyses of all float data with other available data. The
procedures for regional analyses are still to be determined.
...
*Exercise Three* : Use the following code to look at the `<PARAM>_ADJUSTED` vs
`<PARAM>`. EXPLAIN YOUR RESULTS.
...
*Example Four* Use `useAdjusted()` on the float ID=5903586, profile=50 to
determine the % difference between `<PARAM>_ADJUSTED` vs `<PARAM>_UNADUSTED>`.
