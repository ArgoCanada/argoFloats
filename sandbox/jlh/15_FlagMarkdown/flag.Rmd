...
`argoFloats` developed a way to therefore deal with flags and adjusted data. When dealing with flags, a three step process exists involving `plot(which='QC')`, `showQCTests()`and `applyQC()`. In addition to flag control,
`argoFloats` also deals with adjusted data with the `useAdjusted()` function.
