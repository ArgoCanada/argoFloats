...
library(oce)
qf <- function(x) {
    100 * (1 - sum(4 == x[[paste0(item, 'Flag')]]) / length(x[[paste0(item, 'Flag')]]))
}
meanf <- function(x) {
    mean(x[[item, na.rm=TRUE]])
}
if (!exists("bai")) {
...
for (item in items) {
    ...
    if (any(is.finite(q))) {
        ...
        oce.plot.ts(time, m, ylab=paste(item, "Mean"), type='l', col='grey', drawTimeRange = FALSE)
