...
So, they are the same.  Whether that will always be the case is an open
question.  However, before moving on, we ought to double-check that oce is
reading the netCDF correctly:

```{r}
library(ncdf4)
n <- nc_open(file)
for (i in 1:3) {
    stopifnot(all.equal(ncvar_get(n, "PSAL_QC")[i],
                        paste(d[["salinityFlag"]][,i], collapse="")))
    stopifnot(all.equal(ncvar_get(n, "PSAL_ADJUSTED_QC")[i],
                        paste(d[["salinityAdjustedFlag"]][,i], collapse="")))
    stopifnot(all.equal(ncvar_get(n, "TEMP_QC")[i],
                        paste(d[["temperatureFlag"]][,i], collapse="")))
    stopifnot(all.equal(ncvar_get(n, "TEMP_ADJUSTED_QC")[i],
                        paste(d[["temperatureAdjustedFlag"]][,i], collapse="")))
    stopifnot(all.equal(ncvar_get(n, "PRES_QC")[i],
                        paste(d[["pressureFlag"]][,i], collapse="")))
    stopifnot(all.equal(ncvar_get(n, "PRES_ADJUSTED_QC")[i],
                        paste(d[["pressureAdjustedFlag"]][,i], collapse="")))
}
```
