...
load("index.rda")
par(mar=c(2, 2, 1, 1))
oce::mapPlot(coastlineWorldFine, col="tan",
             latitudelim=c(40, 50), longitudelim=c(-70, -50),
             projection="+proj=merc +lon_0=-60")
oce::mapPoints(index[["longitude"]], index[["latitude"]], pch=20, col=rgb(1,0,0,0.1))
mtext(paste(length(index[["longitude"]]), "Argo profiles in data(index)"))
oce::mapContour(topoWorld[["longitude"]], topoWorld[["latitude"]], -topoWorld[["z"]],
