...
overallHelp <- "This GUI has two main tabs, the Main and Settings tab, and a set of keystrokes. <br><br>Type <b>?</b> to bring up a window that lists key-stroke commands for further actions including zooming and shifting the spatial view, sliding the time window, resetting the map to the initial state, and holding a message.<br><br> On the <b> Main tab</b>, enter values in the \"Start\" and \"End\" boxes to set the time range in numeric yyyy-mm-dd format, or empty either box to use the full time range of the data.<br><br>Use \"View\" to select profiles to show (core points are in black, deep in purple, and BGC in green), whether to show coastline and topography in high or low resolution, whether to show contour lines, and whether to show a path trajectory. If a path is displayed, you have the option to highlight the start and end points of the path, or view the path without profile locations. Click-drag the mouse to enlarge a region. Double-click on a particular point to get a popup window giving info on that profile.<br><br>A box above the plot shows the mouse position in longitude and latitude.  If the latitude range is under 90 degrees, a scale bar will appear, and if the mouse is within 100km of a float location, that box will also show the float ID and the cycle (profile) number.<br><br> On the <b> Settings tab</b>, you have the ability to click on <b>Core</b>, <b>BGC</b>, or <b>Deep</b>. Each of these have the option to change the symbol colour, type, and size, as well as the path colour and width. You are also provided with a \"Symbol Gallery\" button which when clicked provides a visual of the relevant symbol types<br><br>The \"R code\" button brings up a window showing R code that isolates to the view shown and demonstrates some further operations.<br><br>For more details, type <tt>?argoFloats::mapApp</tt> in an R console."
