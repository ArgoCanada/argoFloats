...
server <- function(input, output, session)
{
    ...
    if (argoFloats:::argoFloatsIsCached("argo")) {
        ...
        argo <- argo[ok, ]
        argoFloats:::argoFloatsStoreInCache("argo", argo, debug=debug)
    }
    ...
    output$UIwidget <- shiny::renderUI({
        if (argoFloats:::argoFloatsIsCached("argo") && input$tabselected %in% c(1)) {
            shiny::fluidRow(shiny::span(shiny::HTML(paste("<b style=\"color:blue; margin-left:1em;\">  ",appName, appVersion,"</b>"))),
                ...
                shiny::actionButton("helpSymbolCodes", "Symbol Codes"),
    ...
    output$UItrajectory <- shiny::renderUI({
        if (argoFloats:::argoFloatsIsCached("argo") && input$tabselected %in% c(1) && "path" %in% state$view) {
    ...
    shiny::observeEvent(input$helpSymbolCodes,
        {
